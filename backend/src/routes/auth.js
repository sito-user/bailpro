...
router.post('/register', async (req, res, next) => {
  try {
    ...
    if (existingOrg) {
      return res.status(409).json({ error: 'SLUG_TAKEN', message: 'Ce nom d\'organisation est déjà pris' });
    }

    const password_hash = await bcrypt.hash(password, 12);

    const result = await db.transaction(async (trx) => {
      const [org] = await trx('organizations').insert({ name: org_name, slug }).returning('*');
      const [user] = await trx('users').insert({
        org_id: org.id, email, password_hash, full_name, role: 'admin',
